#
# spec file for package kernel-stable
#
...
%define srcversion 6.8.9
%define patchversion 6.8.9
%define git_commit f3d61438b613b87afb63118bea6fb18c50ba7a6b
%define variant -stable%{nil}
...
Version:        6.8.9
Release:        0
Summary:        The standard upstream rolling stable kernel
...
ExclusiveArch:  alpha alpha alpha aarch64 aarch64 aarch64 aarch64 armv6hl armv6hl armv7hl armv7hl armv7hl armv7hl %ix86 %ix86 %ix86 %ix86 ia64 ia64 ia64 mips mips mips mips64 mips64 mips64 parisc parisc parisc ppc ppc ppc ppc64 ppc64 ppc64 ppc64 ppc64le ppc64le ppc64le ppc64le riscv64 riscv64 riscv64 s390 s390 s390 s390x s390x sparc sparc sparc sparc64 sparc64 sparc64 x86_64 x86_64 x86_64 x86_64
...
%define obj_install_dir /usr/src/linux-%{kernelrelease}-vanilla-obj
...
Provides:       multiversion(kernel)
Requires:       kernel-source-stable = %{version}-%{source_rel}
Supplements:    packageand(%{name}:kernel-source-stable)
...
relink ../../linux-%{kernelrelease}-vanilla-obj/"%{cpu_arch_flavor}" /usr/src/linux-obj/"%{cpu_arch_flavor}"
...
# Ugly hack:
sed -i -r 's/(depmod_hack_needed=)true/\1false/' linux-%{version}/scripts/depmod.sh
...
while true; do
    ...
    # package.
    if %{_sourcedir}/try-disable-staging-driver "$log"; then
        echo "Retrying make"
    else
        exit 1
    fi
...
# %{modules_dir}/source points to the source
...
rm -rf %{buildroot}%{modules_dir}/vdso/.build-id
%endif
...
%endif
    ...
    %if 0%{?klp_symbols}
        cp Symbols.list %{rpm_install_dir}/%{cpu_arch_flavor}
        echo %{obj_install_dir}/%{cpu_arch_flavor}/Symbols.list > %{my_builddir}/livepatch-files.no_dir
...
%endif
    ...
    # build symlink.
    rm -f %{buildroot}%{modules_dir}/{source,build}
    ln -s %{src_install_dir} \
    %{buildroot}%{modules_dir}/source
    ln -s %{obj_install_dir}/%{cpu_arch}/%{build_flavor} \
    %{buildroot}%{modules_dir}/build
...
%endif
    ...
    test -s %{buildroot}%{modules_dir}/modules.dep && \
    dd if=/dev/zero of=%{buildroot}%{modules_dir}/modules.dep ibs=$(stat -c%s %{buildroot}%{modules_dir}/modules.dep) count=1
